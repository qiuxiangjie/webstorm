$(function () {
    ...
    /*数据加载*/
    $.ajax({
        url:'http://dms.bnq.com.cn/dms/project/reform/originDetail.share?trackId='+trackId,
        dataType:'json',
        method:'get',
        success:function(data){
            if(data.response.code===0){
                ...
                isNotNull(data.response.data.reformInfo.require.note) && $('#note').text(data.response.data.reformInfo.require.note);
    ...
    function renderPlan(data){
        ...
        if(isNotNull(data.protect)){
            renderScoreDOM(data.protect,'工地成品保护检查');
        }
