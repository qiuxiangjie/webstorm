(function (){
	...
	function getDataCallback(data){
		...
		if(data.houseArea == null){
			...
			$('#houseArea').text(Math.round(data.houseArea) + " ㎡");
		}
		...
		//开始时间/完成时间/天数
		var realStartTime = isNotNull(data.actualStartTimeStr)?data.actualStartTimeStr
			:isNotNull(data.planStartTimeStr)?data.planStartTimeStr
			:'暂无';
		var realEndTime = isNotNull(data.actualEndTimeStr)?data.actualEndTimeStr
			:isNotNull(data.planEndTimeStr)?data.planEndTimeStr
			:'暂无';
		var realDay = isNotNull(data.realDay)?data.realDay:'0';
		...
		//施工情况--工程管理员
		if(data.projectAdminName == null){
			$('#proAdminName').parent().remove();
		} else {
			$('#proAdminName').text(data.projectAdminName);
		}
