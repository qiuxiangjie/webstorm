 ...
 */
$(function () {
    var URL2 = 'http://192.168.251.81:8080/';
    ...
    if(initData.response.code===0){
        if(dsMpa.code===0 && dsMpa.results.length>0){
            initMap();
            renderMap(dsMpa);
        }else{
            initMap();
        ...
        $('#setMeal').text(dsProject.setMealStr?dsProject.setMealStr:'无');//设置套餐
        ...
        $.ajax({
            ...
            success:function(data){
                if(data.response.code===0){
                    ...
                    if(isNotNull(contractProvince)&&isNotNull(contractCity)&&isNotNull(contractDistrict)){
                        $('.map-input div').html('<span></span><span></span><span></span>');
                    }
                    function isNotNull(obj){
                        return (obj !== null && obj !== "undefined" && obj !== "");
                    }
                    if(isNotNull(contractProvince))
                    $('.map-input div').find('span').eq(0).text(contractProvince+'/');
                    if(isNotNull(contractCity))
                    $('.map-input div').find('span').eq(1).text(contractCity+'/');
                    if(isNotNull(contractDistrict))
        ...
        function renderMap (data){
            if(data.code===0){
                ...
                if(mapData.length>0){
                    data.results.forEach(function(item,i){
                        ...
                        if(i==0){
                            ...
                            map.centerAndZoom(new BMap.Point(item.longitude,item.latitude), 15); // 初始化地图，设置中心点坐标和地图级别
                        }
                ...
                alert(data.response.message+'123');
            }
        }
        ...
        function mapSearch (cityName,ser,province, district){
            //var url = URL + appDir+'/project/address/searchMap.do?city='+cityName+'&q='+ser+'&pageSize=10&curPage=1&province='+province+'&district='+district;
            //var url = URL+'./images/map.json';

            var url = URL2 + appDir+'/project/address/searchMap.do?city='+cityName+'&q='+ser+'&pageSize=10&curPage=1&province='+province+'&district='+district;
            $.ajax({
                url:url,
                dataType:'json',
                method:'get',
        ...
        function setMobileLocation(map, lon, lat,Icon){
            var point = new BMap.Point(lon, lat+0.005);
            map.centerAndZoom(point, 15);  //设置中心点
        ...
        function postData(){
            ...
            $.ajax({
                url:URL+appDir+'/project/address/save.do?',
                dataType:'json',
                method:'post',
                data:{
                    "projectId":projectId,
        ...
        function goList(){
            window.location.href=appDir+"/project/address/list.vdo";
        }
    }else{
        alert(initData.response.message);
    }
});
